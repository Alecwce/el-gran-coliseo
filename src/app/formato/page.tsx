    ...
    <div className="mx-auto max-w-5xl px-4 py-16 sm:px-6 lg:px-8">
      ...
      <div className="flex flex-col gap-8 mb-20">
        {formatos.map((item, idx) => (
          <div
            key={idx}
            className="group relative border border-[#D4AF37] bg-[#191B1F] p-8 transition-all hover:shadow-[0_0_20px_rgba(212,175,55,0.2)]"
          >
            <div className="mb-4 flex flex-wrap items-center gap-4">
              <div className="bg-[#0A0A0A] px-4 py-2 font-bold text-[#D4AF37] border border-[#D4AF37]/50 tracking-wider">
                {item.bo}
              </div>
              <span className="text-[#0066FF] font-bold uppercase tracking-widest text-sm">{item.date}</span>
            </div>
            <h2 className="mb-4 font-serif text-3xl font-bold text-white uppercase">{item.title}</h2>
