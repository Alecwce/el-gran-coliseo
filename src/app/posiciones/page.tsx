...
function StandingsTable({ title, data }: { title: string, data: typeof standings.A }) {
  return (
    <div className="border border-[#D4AF37]/50 bg-white dark:bg-[#191B1F] p-6 shadow-xl transition-colors duration-300">
      <h2 className="mb-6 font-serif text-2xl font-bold uppercase text-[#D4AF37] border-b border-[#D4AF37]/30 pb-2">{title}</h2>
        ...
        <table className="w-full text-left text-sm whitespace-nowrap">
          ...
          <tbody>
            {data.map((row, idx) => {
              ...
              return (
                <tr key={row.team} className={`border-b border-gray-200 dark:border-gray-800/50 hover:bg-gray-50 dark:hover:bg-[#201f1f] transition-colors ${isUnbeaten ? 'bg-[#53FC18]/5 dark:bg-[#53FC18]/5' : ''}`}>
                  ...
                  <td className="py-4 px-4">
                    <div className="flex items-center gap-3">
                      <div className="relative h-10 w-10 flex-shrink-0 bg-gray-100 dark:bg-[#0A0A0A] border border-[#D4AF37]/30 rounded p-1">
                        <Image src={`/logos/${logo}`} alt={row.team} fill className="object-contain p-1 drop-shadow-sm" />
                      </div>
                      <span className={`font-bold uppercase tracking-wide ${isUnbeaten ? 'text-[#0066FF] dark:text-[#53FC18]' : 'text-black dark:text-white'}`}>
                        {row.team}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="py-4 px-4 hidden sm:table-cell">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-xs font-bold text-gray-500 dark:text-gray-400 w-10 text-right">{Math.round(winRate)}%</span>
                    </div>
                  </td>
                </tr>
              );
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
      <div className="mb-12 text-center">
        ...
        <p className="mt-4 text-gray-400 text-lg">Clasificación en tiempo real de los Gladiadores rumbo a los Playoffs.</p>
      </div>
